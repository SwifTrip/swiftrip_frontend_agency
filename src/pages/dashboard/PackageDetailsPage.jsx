...
export default function PackageDetailsPage() {
  ...
  const fetchPackageDetails = async () => {
    try {
      ...
      if (result.success) {
        ...
        toast.error(result.message || "Failed to load package details");
      }
    } catch (error) {
  ...
  const tourStays = packageData.tourStays || [];
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">
                  Complete package information and itinerary
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              {/* Only show Manage Schedules for public tours */}
              {packageData.isPublic && (
                <button
                  onClick={() => navigate(`/app/packages/${id}/schedules`)}
              ...
              <button
                onClick={() => navigate(`/app/packages/edit/${id}`)}
        ...
        <div className="max-w-7xl mx-auto px-6 py-6">
                ...
                <div className="flex items-center gap-4 text-sm text-gray-600">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span className="font-medium">
                      {packageData.itineraries?.length || 0} Days
                    </span>
                  </div>
                  <div className="flex items-center gap-1">
                    <svg
          ...
          <div className="grid grid-cols-3 gap-6">
            ...
            <div className="col-span-2 space-y-6">
              ...
              <div className="grid grid-cols-3 gap-4">
                  ...
                  <div className="pt-3 border-t border-white/20 space-y-2 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold">
                        {packageData.currency}{" "}
                        {(packageData.basePrice * 10).toLocaleString()}
                      </span>
                    </div>
                    <div className="flex justify-between">
                ...
                <div className="bg-white rounded-2xl p-6 border border-gray-200 shadow-sm">
                  ...
                  <div className="flex items-center justify-center py-3">
                    <div className="px-4 py-2 bg-gradient-to-r from-purple-600 to-orange-600 text-white rounded-lg font-semibold text-sm">
              ...
              {/* What's Included */}
              <div className="bg-gradient-to-br from-orange-50 to-emerald-50 rounded-2xl p-6 border border-orange-100 shadow-sm">
                ...
                <div className="grid grid-cols-4 gap-6">
                  {[
                    ...
                    { icon: "🚐", label: "Transport", color: "bg-orange-100" },
                    {
                      icon: "🏨",
                      label: "Accommodation",
                      color: "bg-purple-100",
                    ...
                    >
                      <div
                        className={`w-16 h-16 ${item.color} rounded-full flex items-center justify-center text-3xl mb-2 shadow-sm`}
                      >
                        {item.icon}
                      </div>
              ...
              <div className="bg-white rounded-2xl p-6 border border-gray-200 shadow-sm">
                ...
                {tourStays.length === 0 ? (
                  <p className="text-sm text-gray-500">No shared stays added</p>
                ) : (
                  <div className="space-y-3">
                    {tourStays.map((stay) => (
                      <div
                        key={stay.id || `${stay.hotelName}-${stay.checkInDay}`}
                        ...
                        <div className="flex justify-between text-sm">
                          <div>
                            ...
                            <p className="text-gray-600">
                              {stay.stayType} • {stay.rating ?? "—"}★
                            </p>
                          </div>
                          <div className="text-right">
                            <p className="text-gray-600">
                              Days {stay.checkInDay} → {stay.checkOutDay}
                            </p>
                            <p className="text-gray-600">
                              Rooms {stay.rooms} • Beds/Room {stay.bedsPerRoom}
                            </p>
                          </div>
                        </div>
                        {stay.stayDetails && (
                          <div className="mt-2 text-xs text-gray-500">
                            Room: {stay.stayDetails.roomType || "—"} • Check-in{" "}
                            {stay.stayDetails.checkInTime || "—"} • Check-out{" "}
                            {stay.stayDetails.checkOutTime || "—"}
                          </div>
                        )}
                      </div>
                    ))}
              ...
              <div className="bg-white rounded-2xl p-6 border border-gray-200 shadow-sm">
                ...
                packageData.itineraries.length > 0 ? (
                      ...
                      >
                        ...
                        {expandedDay === itinerary.dayNumber && (
                          <div className="p-4 bg-white">
                            {(itinerary.dayStartTime ||
                              itinerary.dayEndTime) && (
                              <div className="mb-3 text-xs text-gray-500">
                                Day time: {itinerary.dayStartTime || "—"} →{" "}
                                {itinerary.dayEndTime || "—"}
                              </div>
                            )}
                            {itinerary.dayTransports &&
                              itinerary.dayTransports.length > 0 && (
                                <div className="mb-4">
                                  <p className="text-xs font-semibold text-gray-700">
                                    Day Transports
                                  </p>
                                  <ul className="mt-1 space-y-1">
                                    {itinerary.dayTransports.map((dt) => (
                                      <li
                                        key={`${
                                          dt.transportId ||
                                          dt.transport?.id ||
                                      ...
                                      >
                                        ...
                                        <span>
                                          {dt.transport?.vehicleType ||
                                            dt.transportDetails?.vehicleType ||
                                            "Vehicle"}{" "}
                                          {dt.isPrimary ? "— Primary" : ""}
                                          {(dt.transport?.startLocation ||
                                            dt.transportDetails
                                              ?.startLocation) &&
                                            (dt.transport?.endLocation ||
                                              dt.transportDetails
                                                ?.endLocation) && (
                                    ...
                                    <div className="flex items-start gap-3">
                                      ...
                                      <div className="flex-1">
                                        <div className="flex items-start justify-between mb-2">
                                          ...
                                          {item.optional && (
                                            <span className="px-2 py-1 bg-orange-100 text-orange-700 text-xs font-medium rounded ml-3 flex-shrink-0">
                                              Optional
                                            </span>
                                          )}
                                        </div>
                                            ...
                                            <div className="flex items-center gap-1">
                                              ...
                                              >
                                                <path
                                                  ...
                                                  strokeLinejoin="round"
                                                  strokeWidth={2}
                                                  d="M9.75 17L8.5 21h7l-1.25-4M4 10h16l-1 5H5l-1-5z"
                                                />
                                              </svg>
                                              <span>
                                                Transport required
                                                {typeof item.transportId ===
                                                "number"
                                                  ? ` (#${item.transportId})`
                                                  : ""}
                                              </span>
                  ...
                  <div className="text-center py-12 text-gray-500">
                    ...
                    <p className="text-xs">
                      This package doesn't have a detailed itinerary yet
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex justify-between items-center pb-3 border-b border-gray-100">
                    ...
                    <span className="text-xl font-bold text-green-600">
                      {packageData.itineraries?.reduce(
                        (acc, day) => acc + (day.itineraryItems?.length || 0),
                        0
                      ) || 0}
                    </span>
                  </div>
              ...
              {/* Quick Actions */}
              {/* <div className="bg-gradient-to-br from-orange-50 to-indigo-50 rounded-2xl p-5 border border-orange-100 shadow-sm">
                ...
                <div className="space-y-2">
                  ...
                  <button 
                    ...
                    className="w-full px-4 py-2.5 bg-orange-600 text-white rounded-lg hover:bg-orange-700 transition-colors text-sm font-medium shadow-sm flex items-center justify-center gap-2"
                  >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="bg-white rounded-2xl p-5 border border-gray-200 shadow-sm">
                ...
                <div className="space-y-3 text-sm">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-medium text-gray-900">
                      {packageData.maxGroupSize} people
                    </span>
                  </div>
                </div>
              </div>
